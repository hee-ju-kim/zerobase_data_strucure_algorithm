...
public class Practice3 {
    ...
    public static boolean chkSplit(int[][] graph, int[] flags, int flag, int node) {
        if (flags[node] != 0) {
            return flags[node] == flag;
        }

        flags[node] = flag;
        for (int adNode: graph[node]) {
            if (!chkSplit(graph, flags, -flag, adNode)) {
                return false;
            }
        }
        return true;
